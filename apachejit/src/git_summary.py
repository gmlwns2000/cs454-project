...
def extract_method(line, summary, curr_class, add_method_cnt, removed_method_cnt):
  ...
  if match:
    ...
    if line.startswith('+'):
      ...
      if curr_class in summary["new_class"]:
        ...
        if "unknown_class" not in summary:
          summary["unknown_class"] = {}
        
        if "add_method" not in summary["unknown_class"]:
          summary["unknown_class"]["add_method"] = []
        summary["unknown_class"]["add_method"].append(curr_method)
      ...
      if curr_class in summary["removed_class"]:
        ...
        if "removed_method" not in summary["unknown_class"]:
          summary["unknown_class"]["removed_method"] = []
        summary["unknown_class"]["removed_method"].append(curr_method)
      removed_method_cnt += 1
      
    return curr_method, summary, add_method_cnt, removed_method_cnt
...
def git_summary(diff):
  ...
  for line in lines:
    ...
    # extract method
    curr_method, summary, add_method_cnt, removed_method_cnt = extract_method(line, summary, curr_class, add_method_cnt, removed_method_cnt)
